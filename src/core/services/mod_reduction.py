...
def modReduction(poly1:str, poly2:str, inputType:str, m:int=163):
    ...
    #If the input is not compatible in the galois field of the input    
            ...
            if int(poly1,2)>2**m:
                if m!=571:
                    ...
                    if inputType == 'binary': #Convert result into binary
                        poly1=bin(int(poly1))[2:].zfill(m)  #Binary string padded to m bits and removing the prefix 
                    elif inputType == 'hexadecimal': #Convert result into hexadecimal
                        poly1=hex(int(poly1))[2:].upper().zfill(m // 4)  #Hex string padded to m/4 characters and removing the prefix
            if int(poly2,2)>2**m:
                if m!=571:
                    irreduciblePoly=gf.irreducible_poly 
                    integerRep=int(irreduciblePoly)
        ...
        while result.bit_length()-1 >= modDegree:
            degreeDiff=result.bit_length() - modDegree - 1 #Degree difference
            result^=fieldPoly2<<degreeDiff #XOR the mod shifted to align with poly's leading term
        return result  
...
def gf2Mod(poly:int, mod:int, inputType:str, m:int): 
    ...
    if inputType == 'binary': #Convert result into binary
            poly = bin(int(poly))[2:].zfill(m)  # Binary string padded to m bits and removing the prefix 
    elif inputType == 'hexadecimal': #Convert result into hexadecimal
            poly = hex(int(poly))[2:].upper().zfill(m // 4)  # Hex string padded to m/4 characters and removing the prefix
